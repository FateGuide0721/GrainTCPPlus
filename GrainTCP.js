...
const mill = async (rd, w) => {
    const reader = rd.getReader({ mode: 'byob' }), tx = mkDn(w); let buf = new ArrayBuffer(CFG.chunk);
        ...
        if (busy || closed) return; busy = true; try {
            for (; ;) {
                if (closed) break; if (!sock) {
                    const [d] = uq.bundle(); if (!d) break; const res = x(d); if (!res) throw wither(); server.send(new Uint8Array([d[0], 0])); const host = addr(res.addrType, res.targetAddrBytes), port = res.port, payload = d.subarray(res.dataOffset);
